...
class SlotsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create slot" do
    assert_difference("Slot.count") do
      post slots_url, params: { slot: { jetty: @slot.jetty, length: @slot.length, width: @slot.width } }
    end
  ...
  test "should update slot" do
    patch slot_url(@slot), params: { slot: { jetty: @slot.jetty, length: @slot.length, width: @slot.width } }
